...
| Light settings editor ⚠ | Inline modal on the dashboard: `#room-config` in `tools/floorplan/dashboard.template.html` + the `cfg*` functions in `web/scripts/home.js`; `GET`/`POST /config/zones` in `mqtt-web.js` — **must stay below `cookieMiddleware`**. Opened from the room-menu's config symbol; `web/config.html` is gone |
| Whole-flat scene editor ⚠ | `#scenes-config` in `tools/floorplan/dashboard.template.html` + the `scenes*` functions in `web/scripts/home.js`; `GET`/`POST /config/scenes` in `mqtt-web.js` — **also must stay below `cookieMiddleware`**. A dropdown per room×scene setting that room's mood (`—`/`off`/a step); the scene `off` is not editable. Detail: `docs/mqtt-and-devices.md` |
