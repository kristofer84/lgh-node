...
`light.turn_on` with no brightness, and zwave-js writes **`targetValue 255`**, which Multilevel
Switch defines as *restore previous level*. The node even publishes a `restorePrevious` topic
saying so. Caught on the wire 2026-08-29 on `light.sovrum_4_tak`: `targetValue 255` at
05:25:03.874Z, `currentValue` **10** at 05:25:04.928Z — the lamp went back to the 10 % a
...
21 s, with reports still arriving 16 s after the last command — the far kitchen nodes are four
hops out (koksbank 98, kokso 100) with round trips up to 1.4 s. In that window a `currentValue`
report generated by the *previous* step can be delivered after the new one and leave the model
showing the old level. Two separate investigations of "kökshylla did not dim right" ended here:
commanded alone it is flawless (100 → 99, 10 → 10, twice), and after reproducing the two-step
press and polling the device directly — `pollValue` over the zwave-js-ui MQTT API,
`zwave/_CLIENTS/ZWAVE_GATEWAY-Zwavejs2Mqtt/api/pollValue/set`, which **queries the node** rather
than returning zwave-js's cache — the hardware answered **99** while nothing was wrong at all.
⚠ **Use `pollValue`, not a cached read, whenever a level is in doubt**; a cached read just
echoes the value you are trying to check. (`node_status`/`restored`/`_last_seen` answer whether
a device is *alive*; this answers what it is actually *doing*.)
The app makes this worse than it needs to be: a room press publishes to **every** device in the
zone, including lights already in the target state, so three of those five kitchen writes were
`off` sent to lights that were already off. Skipping no-op publishes would cut the burst — not
done, because the model would then be deciding to stay silent, and a stale model would silently
skip a real command.
